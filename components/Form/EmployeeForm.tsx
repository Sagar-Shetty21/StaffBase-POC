...
export default function EmployeeForm({ onSubmit }: EmployeeFormProps) {
  ...
  const handleAddSkill = async () => {
    const isValid = await trigger("new_skill_temp");
    if (!isValid) return;
            ...
            <div className={styles.inputRow}>
              <input
                ...
                {...register("new_skill_temp", {
                  ...
                  validate: (value) => {
                    const exists = (skillsValue || []).some(
                      (s) => s.toLowerCase() === value.trim().toLowerCase()
                    );
                    return exists ? "This skill already exists" : true;
                  },
                })}
                className={`${styles.input} ${errors.new_skill_temp ? styles.error : ""}`}
